...
// Generated 07-Jul-2021 20:36:48
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"margin\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(margin_M, (NULL));
        margin_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(margin_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(margin_M)
                == (NULL)) || rtmGetStopRequested(margin_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(margin_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
